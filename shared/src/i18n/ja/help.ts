...
const help: TranslationStrings = {
  ...
  // files-sync
  'help.guide.files-sync.title': 'ドキュメントを Nextcloud のフォルダーと同期し続ける',
  'help.guide.files-sync.goal':
    '旅行を自分のドキュメント保管先のフォルダーに結び付け、ここでアップロードしたものが向こうに届き、向こうで整理したものがここに現れるようにします。',
  'help.guide.files-sync.step.1':
    'ツールバーの右端、「ゴミ箱」の隣の「ドキュメント同期」をクリックします。ダイアログがタイトルの下に旅行の名前を付けて開きます。左の「連携先を接続」の下には管理者がオンにした保管先が並び、それぞれに整理の仕方の1行が付きます。Nextcloud なら「フォルダーで整理」です。右には「まだ何も接続されていません」と読めます。',
  'help.guide.files-sync.step.2':
    'Nextcloud をクリックします。接続のための小さなダイアログが保管先の名前で開き、上部にアカウントのパスワードではなくアプリパスワードを使うようにという注意があります。',
  'help.guide.files-sync.step.3':
    '「アドレス」、「ユーザー名」、つまりメールアドレスではなく Nextcloud のログイン名、そして Nextcloud が設定のセキュリティで作る「アプリパスワード」を入力します。「基準フォルダー」は任意です。TREK が旅行用フォルダーを探す場所で、指定しなければ /TREK、ここでは /Reisen です。一番下の「自己署名証明書を許可」は、そのような証明書を持つ自分のネットワーク上のインスタンスのためだけのものです。',
  'help.guide.files-sync.step.4':
    '「接続をテスト」をクリックします。TREK は入力した内容で保管先に到達し、フッターは「接続できました。」に続けて、アカウント名を入れた「… としてサインインしています」と読めます。ここでは admin です。拒否されたパスワードや到達できないアドレスは代わりにそこに示され、どちらの場合も何も保存されません。',
  'help.guide.files-sync.step.5':
    '「接続」をクリックします。接続は旅行とともに保存され、TREK は「この旅行を Nextcloud のどこに置きますか？」と尋ねます。同期されるのはそのフォルダーの中身だけです。「新しく作る」は「作成」で基準フォルダーの下にフォルダーを作り、名前は旅行のタイトルからあらかじめ入っています。「または既にあるものを使う」の下にはすでにあるフォルダーが並びます。1つをクリックします。ここでは Autumn in Japan です。',
  'help.guide.files-sync.step.6':
    'ダイアログが戻ってきます。左の「この旅行」の下に Nextcloud があり、右のそのカードにはフォルダー、最後に実行した時刻、「今すぐ同期」があります。最初の実行はひとりでに始まり、「今すぐ同期」はいつでも1回実行します。1回の実行が終わると、名前の隣の「まだ同期していません」のバッジは緑の点に変わり、ポインターを重ねると「同期済み」と読めます。フローバーは TREK と Nextcloud がそれぞれ持つドキュメントを数え、その間に「保管先へ」と「保管先から」のレーンがあります。× でダイアログを閉じます。',
  'help.guide.files-sync.result':
    'フォルダーにすでにあった2つのドキュメントが、あなたの名前でアップロードされたものとしてリストの先頭に並び、旅行のすべてのドキュメントもフォルダーにあります。以後、TREK は背後でフォルダーを確認し、フォルダーはリストに従います。',
  'help.guide.files-sync.tip.1':
    '旅行を結び付けられるのは旅行のオーナーかインスタンスの管理者だけです。アプリパスワードがその Nextcloud アカウント全体に及ぶからです。メンバーは誰でも「ドキュメント同期」を開き、カードを読み、「今すぐ同期」を押せます。',
  ...
  'help.guide.files-sync.tip.3':
    'カードの「接続解除」は結び付きを終え、両側のすべてのドキュメントを保ちます。二度目に結び付けたフォルダーは新しいものとして扱われ、中身がすべてもう一度入ってくるので、「接続解除」のあとは古いフォルダーではなく空のフォルダーを結び付けてください。',
  ...
  'help.guide.import-booking-file.step.5':
    '見つかった予約はそれぞれ「新しい予約」に、内容が入った状態で順に開きます。ホテルなら「タイトル」に名前、旅行にその場所があれば「宿泊先」の下にその場所、「場所／住所」、宿泊日に合わせた「開始」と「終了」、「チェックイン」と「チェックアウト」、「予約コード」、「ファイル」の下に確認書、そして「費用」がオンなら「Linked expense」として料金です。確かめて「追加」をクリックします。',
  ...
  'help.guide.expense-from-booking.step.2':
    'フォームの一番下のCostsのブロックまでスクロールします。まだ何も紐づいていなければCreate expenseが出ていて、先に予約を保存すると書かれています。',
  'help.guide.expense-from-booking.step.3':
    'Create expenseをクリックします。予約が保存され、フォームが閉じ、予約のタイトルを名前として、種類に合うカテゴリがすでに当てられた状態で費用のエディタが開きます。',
  'help.guide.expense-from-booking.step.4':
    'ほかの支出と同じように金額とその通貨、支払った人、分割を入力して保存します。予約を開き直すと、Linked expenseの下にそれが出て、編集する鉛筆と外すゴミ箱が付きます。',
  'help.guide.expense-from-booking.result':
    '予約はその費用を持ち、支出は「費用」タブのふつうの行になります。ほかの支出と同じく、支払者と分割と通貨を持ちます。',
  'help.guide.expense-from-booking.tip.1':
    '予約を削除すると、紐づいた支出も一緒に消えます。予約のCostsブロックにあるRemove expenseはその逆で、支出が消えて予約は残ります。',
  'help.guide.expense-from-booking.tip.2':
    '場所のフォームにも同じブロックがあり、Create expenseは先に場所を保存します。',
  ...
  'help.guide.add-transport.tip.2':
    '「ファイル」の下の「ファイルを添付」がチケットを受け取り、「Costs」の下の「Create expense」は予約を保存して運賃のために「費用」のエディターを開きます。',
