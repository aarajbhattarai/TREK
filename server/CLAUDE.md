...
- **DB** (`src/db/`): `better-sqlite3`, WAL, FK on. `database.ts` only opens the connection now; the schema comes from the **MikroORM** migrations in `db/migrations/` and the seeders in `db/seeders/`, applied by `db/orm.ts` from `buildApp()` before anything reads. `db/orm-driver.ts` hands the ORM that same connection instead of opening a second one — with `:memory:` under test, a second connection is a second, empty database. `schema.ts`/`migrations.ts`/`seeds.ts` still exist for the test suite only, guarded by `tests/unit/db/schema-parity.test.ts`. `NODE_ENV=test` ⇒ isolated `:memory:` DB per vitest worker, opened from a snapshot `tests/global-setup.ts` migrates once per run; `TREK_DB_FILE` ⇒ explicit file DB (Playwright harness).
