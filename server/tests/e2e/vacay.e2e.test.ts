...
describe('Vacay e2e (real auth guard + temp SQLite)', () => {
  ...
  async function build() {
    const moduleRef = await Test.createTestingModule({ imports: [await TestUnitOfWorkModule.forRoot(db), await createTestMikroOrmModule(db), DatabaseModule, RealtimeModule, VacayModule] }).compile();
