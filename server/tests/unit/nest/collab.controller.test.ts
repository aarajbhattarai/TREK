...
function svc(o: Partial<CollabService> = {}): CollabService {
  return {
    verifyTripAccess: vi.fn().mockResolvedValue({ user_id: 1 }),
...
describe('CollabController (parity with the legacy /api/trips/:tripId/collab route)', () => {
  describe('notes', () => {
    it('GET lists', async () => {
      const s = svc({ listNotes: vi.fn().mockResolvedValue([{ id: 1 }]) } as Partial<CollabService>);
      expect(await new CollabController(s, storageStub).listNotes(user, '5')).toEqual({ notes: [{ id: 1 }] });
    });

    it('POST creates + broadcasts + notifies (empty title now 400s in the Zod pipe)', async () => {
  ...
  describe('note file filter', () => {
    ...
    it('takes an ordinary attachment', async () => {
      expect(run('itinerary.pdf', 'application/pdf').ok).toBe(true);
    });

    it('refuses the spellings the download route would serve as a document', async () => {
  ...
  describe('chat image filter', () => {
    ...
    it('refuses a name whose extension disagrees with the type it claims', async () => {
  ...
  describe('link preview guard chain', () => {
    ...
    it('resolves the trip and refuses a caller who cannot reach it', async () => {
      expect(guardsOn(CollabController.prototype.linkPreview)).toContain(TripAccessGuard);
    });

    it('sits behind the same authentication as the rest of the controller', async () => {
      expect(guardsOn(CollabController)).toContain(JwtAuthGuard);
    });

    it('demands no write permission, matching the other read routes', async () => {
    ...
    it('the multipart write routes carry no guard decorators, and check inside instead', async () => {
