 ...
 * cache actually does: the container PermissionsService singleton (the MCP
 * _shared helper reaches it through injection, not a separate bridge
 * instance — permissions.bridge no longer exists, see auth.service.ts:116),
 * and the plain backup restore path (backup.impl.ts), which is free
 * functions by design and has nothing to hang an injected service off. A
 * restore rewrites app_settings under the request path's feet; invalidating
 * here is what keeps every reader consistent afterwards.
 */
